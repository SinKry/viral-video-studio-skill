...
```bash
# 创建 Base
lark-cli base +base-create --as user --name "自媒体爆款案例库" --time-zone Asia/Shanghai
```
...
字段类型速查：1=文本, 2=数字, 3=单选, 4=多选, 5=日期, 15=超链接
...
# 写入记录
lark-cli base +record-upsert --as user --base-token "$BASE_TOKEN" --table-id "$TABLE_ID" --json '{
  "fields": {
    "标题": "xxx",
...
- **查询已有案例**：`lark-cli base +record-search --as user --base-token "$BASE_TOKEN" --table-id "$TABLE_ID" --query "关键词"`
- **按类别筛选**：创建视图按"视频类别"分组
- **按评分排序**：创建视图按"拆解评分"降序排列
- **获取资产库链接**：`lark-cli base +base-get --as user --base-token "$BASE_TOKEN"`
...
3. **Base 权限问题**：lark-cli strict-mode 为 bot 且 bot 缺少 `base:app:create` / `base:record:create` scope 时，用 `lark-cli api POST /open-apis/bitable/v1/apps` 和 `lark-cli api POST .../records` 直接调原生 API 可绕过。详见 `references/feishu-bitable-api-patterns.md`。
4. **Bitable 日期字段**：写入日期必须用 Unix 毫秒时间戳（如 `1781020800000`），不能传字符串 `"2026-06-10"`。
5. **Bitable 字段重命名**：`lark-cli api PUT .../fields/{id}` 传 `field_name` 在某些情况下不生效（返回 success 但名称未变）。建议首次建表时直接用正确的字段名创建，不要依赖重命名。
